# Package index

## Bars

Functions for vertical, horizontal, diverging, grouped, and edge bar
charts.

- [`wjp_bars()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_bars.md)
  **\[experimental\]** : Plot a Bar Chart following WJP style guidelines
- [`wjp_divbars()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_divbars.md)
  **\[experimental\]** : Plot a Diverging Horizontal Bar Chart following
  WJP style guidelines
- [`wjp_groupbars()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_groupbars.md)
  **\[experimental\]** : Plot a Grouped Stacked Bar Chart following WJP
  style guidelines
- [`wjp_edgebars()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_edgebars.md)
  **\[experimental\]** : Plot a Horizontal Edgebars Chart following WJP
  style guidelines

## Dots and Lines

Functions for comparing points, trends, slopes, changes, and lollipops.

- [`wjp_dots()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_dots.md)
  **\[experimental\]** : Plot a Dots Chart following WJP style
  guidelines
- [`wjp_lines()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_lines.md)
  **\[experimental\]** : Plot a Line Chart following WJP style
  guidelines
- [`wjp_slope()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_slope.md)
  **\[experimental\]** : Plot a Slope Chart following WJP style
  guidelines
- [`wjp_dumbbells()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_dumbbells.md)
  **\[experimental\]** : Plot a Dumbbell Chart following WJP style
  guidelines
- [`wjp_lollipops()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_lollipops.md)
  **\[experimental\]** : Plot a Lollipop Chart following WJP style
  guidelines

## Radial and Special

Functions for radial charts, rose charts, and gauges.

- [`wjp_radar()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_radar.md)
  **\[experimental\]** : Plot a Radar Chart following WJP style
  guidelines
- [`wjp_rose()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_rose.md)
  **\[experimental\]** : Plot a Rose Chart following WJP style
  guidelines
- [`wjp_gauge()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_gauge.md)
  **\[experimental\]** : Plot a Gauge Chart following WJP style
  guidelines

## Validation, Analysis, and Styling

Support tools for validating data, checking dependencies, analyzing
differences, and applying WJP styling.

- [`wjp_check_data()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_check_data.md)
  : Validate Data Structure for WJPr Charts
- [`wjp_check_deps()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_check_deps.md)
  : Check WJPr Dependencies
- [`diffmeans()`](https://worldjusticeproject-org.github.io/WJPr/reference/diffmeans.md)
  : Difference of Means Analysis
- [`wjp_fonts()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_fonts.md)
  : Load WJP Fonts
- [`wjp_palette()`](https://worldjusticeproject-org.github.io/WJPr/reference/wjp_palette.md)
  : WJP Color Palette
- [`WJP_theme()`](https://worldjusticeproject-org.github.io/WJPr/reference/WJP_theme.md)
  : WJP ggplot2 Theme

## Data

Datasets included in the package.

- [`gpp`](https://worldjusticeproject-org.github.io/WJPr/reference/gpp.md)
  : GPP Sample Data
- [`roli`](https://worldjusticeproject-org.github.io/WJPr/reference/roli.md)
  : Rule of Law Index Historical Data
